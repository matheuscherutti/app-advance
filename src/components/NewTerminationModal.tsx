...
function SortableGridFileItem({
  file,
  index,
  selectedFileId,
  setSelectedFileId,
  removeFile,
}: SortableGridFileItemProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    ...
    >
      ...
      <p
        className="text-[9px] font-bold text-slate-500 truncate mt-1 text-center w-full block px-0.5"
        title={file.name}
      >
        {index + 1}. {file.name}
      </p>
...
export default function NewTerminationModal({ onClose }: ModalProps) {
  ...
  const isFgtsExcluded =
    formData.modality === "" ||
    EXCLUDE_FGTS_MODALITIES.includes(formData.modality);
  ...
  useEffect(() => {
    if (formData.terminationDate) {
      ...
      const [year, month, day] = formData.terminationDate
        .split("-")
        .map(Number);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert(
        "Ocorreu um erro ao processar o arquivo PDF. Verifique se o arquivo não está corrompido ou protegido.",
      );
    } finally {
  ...
  const modalities = [
    ...
    {
      id: "pedido_demissao_aviso_trabalhado",
      name: "Pedido de demissão com aviso trabalhado",
    },
    {
      id: "pedido_demissao_desconto_aviso",
      name: "Pedido de demissão com desconto do aviso",
    },
    {
      id: "pedido_demissao_sem_desconto_aviso",
      name: "Pedido de demissão sem desconto do aviso",
    },
    {
      id: "rescisao_antecipada_experiencia_empregado",
      name: "Rescisão antecipada do contrato de experiência a pedido do empregado",
    },
    {
      id: "rescisao_antecipada_experiencia_empregador",
      name: "Rescisão antecipada do contrato de experiência pelo empregador",
    },
    { id: "rescisao_domestica", name: "Rescisão Domestica" },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm print:relative print:block print:w-auto print:h-auto print:p-0 print:bg-white print:backdrop-blur-none">
      <div
        className={`bg-white w-full ${hasAttachments ? "max-w-7xl lg:h-[90vh]" : "max-w-4xl max-h-[90vh]"} overflow-y-auto lg:overflow-hidden rounded-2xl shadow-2xl flex flex-col print:hidden transition-all duration-300`}
      >
        ...
        <div className="flex flex-col lg:flex-row flex-1 overflow-hidden min-h-0">
          ...
          <div
            className={`flex-1 flex flex-col md:flex-row min-h-0 ${hasAttachments && activeTab !== "form" ? "hidden lg:flex" : "flex"}`}
          >
            {/* Form Section */}
            <div
              className={`flex-1 p-8 border-r border-slate-100 ${hasAttachments ? "lg:overflow-y-auto" : ""}`}
            >
              ...
              <form className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const excludeFgts =
                        nextModality === "" ||
                        EXCLUDE_FGTS_MODALITIES.includes(nextModality);
                      setFormData({
                        ...formData,
                        modality: nextModality,
                        fgtsPenalty: excludeFgts ? "0" : "",
                        fgtsIncludesMonthPrior: excludeFgts
                          ? false
                          : formData.fgtsIncludesMonthPrior,
                        fgtsIncludesConsignment: excludeFgts
                          ? false
                          : formData.fgtsIncludesConsignment,
                      });
                    }}
                  >
                    {modalities.map((m) => (
                      ...
                      <div className="absolute z-20 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-lg p-3 space-y-2.5">
                        <label className="flex items-center gap-2.5 text-xs font-semibold text-slate-700 cursor-pointer select-none hover:bg-slate-50 p-1.5 rounded-lg transition-colors">
                          <input
                            ...
                            onChange={(e) =>
                              setFgtsNotAvailable(e.target.checked)
                            }
                          />
                          Não disponível
                        </label>
                        <label className="flex items-center gap-2.5 text-xs font-semibold text-slate-700 cursor-pointer select-none hover:bg-slate-50 p-1.5 rounded-lg transition-colors">
                          <input
                            ...
                            onChange={(e) =>
                              setFgtsOpenGuides(e.target.checked)
                            }
                          />
                          Guias em aberto
                        </label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-slate-700">
                      Valor Multa{" "}
                      {formData.modality === "acordo_partes" ? "20%" : "40%"}{" "}
                      FGTS
                    </label>
                    <input
                      type="number"
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          fgtsPenalty: e.target.value,
                        })
                      }
                    />
                    <div className="flex gap-4 mt-2">
                      <label
                        className={`flex items-center gap-2 text-[11px] font-medium text-slate-600 ${isFgtsExcluded ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
                      >
                        <input
                          type="checkbox"
                          className="rounded border-slate-300 text-blue-600 focus:ring-blue-500 disabled:cursor-not-allowed"
                          checked={
                            !isFgtsExcluded && formData.fgtsIncludesMonthPrior
                          }
                          disabled={isFgtsExcluded}
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              fgtsIncludesMonthPrior: e.target.checked,
                            })
                          }
                        />
                        FGTS Mês Anterior
                      ...
                      >
                        <input
                          ...
                          className="rounded border-slate-300 text-blue-600 focus:ring-blue-500 disabled:cursor-not-allowed"
                          checked={
                            !isFgtsExcluded && formData.fgtsIncludesConsignment
                          }
                          disabled={isFgtsExcluded}
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              fgtsIncludesConsignment: e.target.checked,
                            })
                          }
                        />
                        Parcela Consignado
                ...
                <div className="border-t border-slate-100 pt-4">
                  <label className="block text-sm font-semibold text-slate-800 mb-2">
                    Arquivos PDFs Anexados (Complementares)
                  </label>
                    ...
                    <div className="text-center space-y-1">
                      ...
                      <p className="text-xs font-semibold text-slate-700">
                        {isProcessingPdf
                          ...
                          : "Clique ou arraste para anexar PDFs"}
                      </p>
              ...
              <div className="flex-1 flex flex-col min-h-0 space-y-6 text-sm">
                <div className="p-4 bg-white rounded-xl border border-slate-200 shadow-sm">
                  ...
                  <p className="text-xs text-slate-400 mt-1 italic">
                    Vencimento original: {calcData.originalPaymentDate}
                  </p>
                </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {!isFgtsExcluded && (
                      <div className="flex justify-between text-slate-600">
                        ...
                        <span className="font-bold text-rose-600">
                          {formatCurrency(formData.fgtsPenalty)}
                        </span>
                      </div>
                    )}
                  </div>
          ...
          {hasAttachments && (
            <div
              className={`w-full lg:w-[450px] xl:w-[500px] border-l border-slate-100 bg-slate-50 flex flex-col min-h-0 ${activeTab !== "pdf" ? "hidden lg:flex" : "flex"}`}
            >
      ...
      <div
        className="hidden print:block font-sans text-slate-900 bg-[#fafaf9]"
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-[0.035]">
            <div className="scale-[2.2]">
              <svg width="200" height="200" viewBox="0 0 200 200" fill="none">
                ...
                <path
                  d="M 140 60 A 65 65 0 1 0 160 110"
                  stroke="#001bb3"
                  strokeWidth="14"
                  strokeLinecap="round"
                />
          ...
          <div className="w-full bg-white pl-12 pr-10 py-4 flex items-center justify-between text-slate-800 border-b-2 border-[#fca311] relative z-10 shadow-sm">
            <div className="flex items-center gap-4">
              <img
                src="/logo_novo.jpg"
                alt="Advance Contabilidade"
                className="h-14 w-auto object-contain"
              />
            </div>
            <div className="text-right">
              <span className="text-[11px] text-[#001bb3] font-black uppercase tracking-[0.15em]">
                Consultoria Empresarial
              </span>
              <p className="text-[7.5px] text-slate-500 uppercase font-bold tracking-widest mt-0.5">
                Soluções Inteligentes e Seguras
              </p>
            </div>
          </div>
          ...
          <div className="pl-14 pr-10 py-7 relative z-10 flex-1 flex flex-col justify-between box-border">
            <div className="flex-1">
              {/* Title */}
              <h1 className="text-center text-xs font-bold uppercase tracking-[0.2em] text-slate-500 mb-1">
                Roteiro de Orientação Operacional
              </h1>
              ...
              <div className="mb-5 leading-relaxed text-[12px] text-slate-700 bg-white p-4 rounded-xl border border-slate-200/50 shadow-sm relative overflow-hidden">
                ...
                <p className="mb-2">
                  Olá, tudo bem? Segue anexo os documentos de{" "}
                  ...
                  , com data de desligamento em{" "}
                  <span className="text-[#001bb3] font-bold">
                    {formatDateStr(formData.terminationDate)}
                  </span>
                  .
                </p>
                <p className="mb-2">
                  A data limite para o pagamento das verbas rescisórias e
                  entrega de vias é{" "}
                  <span className="text-[#001bb3] font-bold">
                    {formatDateStr(customPaymentDate)}
                  </span>
                  .
                </p>
                <p className="text-[#e27c00] font-bold flex items-center gap-1.5 mt-2 text-[11px]">
                  ⚠️ Importante: Providenciar exame demissional do{" "}
                  {formData.modality === "termino_estagio"
                    ? "estagiário"
                    : "colaborador"}
                  .
              ...
              <p className="mb-4 font-extrabold text-[11.5px] text-slate-800 uppercase tracking-wider">
                Fazer a impressão e distribuição das seguintes vias:
              </p>
              {/* Lists */}
                    ...
                    {formData.modality === "termino_estagio" ? (
                      <>
                        <li className="flex items-start gap-2 text-[11px] text-slate-600 leading-tight">
                          ...
                          <span>
                            01 via – Relatório analítico do cálculo de rescisão;
                          </span>
                        </li>
                        <li className="flex items-start gap-2 text-[11px] text-slate-600 leading-tight">
                          <div className="w-3.5 h-3.5 rounded border border-slate-300 bg-slate-50 flex-shrink-0 mt-0.5" />
                          <span>
                            01 via – Termo de rescisão do contrato de estágio;
                          </span>
                        </li>
                        <li className="flex items-start gap-2 text-[11px] text-slate-600 leading-tight">
                          <div className="w-3.5 h-3.5 rounded border border-slate-300 bg-slate-50 flex-shrink-0 mt-0.5" />
                          <span>
                            01 via – Comprovante de pagamento das verbas (Ao
                            fazer o pagamento anexar o comprovante)
                          </span>
                        </li>
                      </>
                    ) : formData.modality === "rescisao_domestica" ? (
                      <>
                        <li className="flex items-start gap-2 text-[11px] text-slate-600 leading-tight">
                          <div className="w-3.5 h-3.5 rounded border border-slate-300 bg-slate-50 flex-shrink-0 mt-0.5" />
                        ...
                        <li className="flex items-start gap-2 text-[11px] text-slate-600 leading-tight">
                          <div className="w-3.5 h-3.5 rounded border border-slate-300 bg-slate-50 flex-shrink-0 mt-0.5" />
                          <span>
                            01 via – Comprovante de pagamento das verbas (anexar
                            na rescisão).
                          </span>
                        </li>
                      </>
                      ...
                      <>
                        ...
                        {hasFgtsGuia && (
                          <>
                            <li className="flex items-start gap-2 text-[11px] text-slate-600 leading-tight">
                              <div className="w-3.5 h-3.5 rounded border border-slate-300 bg-slate-50 flex-shrink-0 mt-0.5" />
                              <span>
                                01 via – Guia de recolhimento de FGTS;
                              </span>
                            </li>
                            <li className="flex items-start gap-2 text-[11px] text-slate-600 leading-tight">
                              <div className="w-3.5 h-3.5 rounded border border-slate-300 bg-slate-50 flex-shrink-0 mt-0.5" />
                        ...
                        {hasSeguroDesemprego && (
                          <li className="flex items-start gap-2 text-[11px] text-slate-600 leading-tight">
                            <div className="w-3.5 h-3.5 rounded border border-slate-300 bg-slate-50 flex-shrink-0 mt-0.5" />
                            <span>
                              01 via - Requerimento de seguro desemprego (Na
                              segunda pagina recortar na linha pontilhada,
                              colher a assinatura do empregado e manter com a
                              empresa)
                            </span>
                          </li>
                        )}
                    ...
                    {formData.modality === "termino_estagio" ? (
                      ...
                      <>
                        ...
                        <li className="flex items-start gap-2 text-[11px] text-slate-600 leading-tight">
                          <div className="w-3.5 h-3.5 rounded border border-slate-300 bg-slate-50 flex-shrink-0 mt-0.5" />
                          <span>
                            01 via – Comprovante de pagamento das verbas.
                          </span>
                        </li>
                      </>
                    ) : (
                      <>
                        ...
                        {hasSeguroDesemprego && (
                          <li className="flex items-start gap-2 text-[11px] text-slate-600 leading-tight">
                            <div className="w-3.5 h-3.5 rounded border border-slate-300 bg-slate-50 flex-shrink-0 mt-0.5" />
                            <span>
                              01 via - Requerimento de seguro desemprego (menos
                              a parte recortada na linha pontilhada)
                            </span>
                          </li>
                        )}
                      </>
              ...
              {/* Summary Values */}
              <div className="bg-gradient-to-r from-[#00129a] to-[#111827] text-white print:text-slate-800 p-5 rounded-xl border border-[#001bb3]/35 print:border-slate-350 shadow-md flex justify-between gap-4 mb-5 relative overflow-hidden print:bg-white print:bg-none print:shadow-none">
                <div className="absolute right-0 top-0 w-32 h-32 bg-[#001bb3]/10 rounded-full blur-2xl pointer-events-none print:hidden" />

                <div
                  className={`flex-1 ${!isFgtsExcluded ? "border-r border-slate-800/80 print:border-slate-200 pr-4" : ""} last:border-0 last:pr-0`}
                >
                  <span className="text-[10px] uppercase tracking-wider text-slate-400 print:text-slate-500 font-bold block">
                    {formData.modality === "termino_estagio"
                      ? "VALOR LÍQUIDO A SER PAGO PARA ESTAGIÁRIO"
                      : "Valor Líquido a ser Pago ao Colaborador"}
                  </span>
                  <div className="text-xl font-black text-[#fca311] print:text-[#00129a] mt-0.5 leading-none flex items-center flex-wrap">
                    ...
                    {(parseFloat(formData.value) === 0 ||
                      ...
                      <span className="text-[10px] text-red-400 print:text-red-750 font-black uppercase tracking-wider ml-2">
                        - Não há valores a serem pagos para o{" "}
                        {formData.modality === "termino_estagio"
                          ? "estagiário"
                          : "funcionário"}{" "}
                        (Rescisão zerada / com estouro)
                      </span>
                    )}
                  </div>
                </div>
                ...
                {!isFgtsExcluded && (
                  <div className="flex-1 pl-4">
                    ...
                    <div className="text-xl font-black text-[#fca311] print:text-[#00129a] mt-0.5 leading-none">
                      {formatCurrency(formData.fgtsPenalty)}
                    </div>
                    {(formData.fgtsIncludesMonthPrior ||
                      formData.fgtsIncludesConsignment) && (
                      <div className="text-[10px] mt-2 py-1 px-2.5 inline-block italic normal-case text-slate-900 font-bold bg-[#fca311] print:bg-slate-100 print:text-slate-900 rounded border border-[#001bb3]/20 print:border-slate-300">
                        está incluso:{" "}
                        {[
                          formData.fgtsIncludesMonthPrior &&
                            "FGTS mês anterior",
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                <p className="text-[11px] text-slate-500 font-medium italic text-center">
                  Orientamos que o pagamento seja feito através de Ordem de
                  pagamento, transferência bancária, cheque administrativo ou
                  Pix.
                </p>
              ...
              {(hasSeguroDesemprego ||
                ...
                formData.modality === "rescisao_domestica") && (
                <div
                  className={`grid ${(hasSeguroDesemprego || formData.modality === "rescisao_domestica") && (!isFgtsExcluded || formData.modality === "rescisao_domestica") ? "grid-cols-2" : "grid-cols-1"} gap-3 text-[11px] bg-white p-3.5 rounded-xl border border-slate-200/50`}
                >
                  {(!isFgtsExcluded ||
                    formData.modality === "rescisao_domestica") && (
                    <p className="text-[#001bb3] font-bold flex items-center gap-1.5 justify-center">
                      <svg
                        className="w-4 h-4 flex-shrink-0 text-[#001bb3]"
                        fill="none"
                        stroke="currentColor"
                        strokeWidth="2"
                        viewBox="0 0 24 24"
                  ...
                  {(hasSeguroDesemprego ||
                    ...
                    <p className="text-[#001bb3] font-bold flex items-center gap-1.5 justify-center">
                      ...
                      Seguro-Desemprego: no App &quot;Carteira de Trabalho
                      Digital&quot;
                    </p>
                  )}
                </div>
              ...
              <div className="bg-amber-50/80 p-3 text-center text-[10.5px] font-black uppercase text-[#991b1b] border border-red-200/60 rounded-xl">
                ATENÇÃO: O PAGAMENTO DA RESCISÃO FORA DO PRAZO ACARRETARÁ EM
                MULTA DE UM SALÁRIO DO{" "}
                {formData.modality === "termino_estagio"
                  ? "ESTAGIÁRIO"
                  : "COLABORADOR"}
                .
